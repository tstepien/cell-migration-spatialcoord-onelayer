function [param_new,min_quantity,exitflag] = batchparamestim4_multipleexplants2(Fk,kb,alpha,rho0,nameofsavedfile)
...
%     for j = 1:length(kb)
        for n = 1:length(alpha)
            for p = 1:length(rho0)
                step = step + 1;
                param.Fk = Fk;%repmat(Fk(i),1,numexplants);
                param.kb = kb;%repmat(kb(j),1,numexplants);
                param.alpha = alpha(n);
                param.rho0 = rho0(p);
