function min_quantity = paramestimation_Fk_kb_varies(param)
% min_quantity = paramestimation_Fk_kb_varies(param)
%
% parameter estimation file that contains minimization function
%
% input:
%   param = parameters (vector with Fkintcpt,Fkslope,kbintcpt,kbslope)
...
if param(1)+param(2)*time.end<=0 || param(3)+param(4)*time.end<=0
    min_quantity = 10^20;
    return
end

simparam.Fkintcpt = param(1);
simparam.Fkslope = param(2);
simparam.kbintcpt = param(3);
simparam.kbslope = param(4);
simparam.rho0 = rho0;

[density,curve] = onelayer_Fk_kb_varies(simparam);
