function [Vx,Vy,Veta,Vzeta,Gs] = cartesian_velocity_anisotropic(KB,Phi,C)
% [Vx,Vy,Veta,Vzeta,Gs] = cartesian_velocity_anisotropic(KB,Phi,C)
%
% Computes the velocity of one layer in a region surrounding the boundary
% in the four Cartesian directions. Allows for nonuniform anisotropic
% diffusion.
%
% inputs:
...
if k > 0
    for m = 1:k
        ...
        if Mark(j,i) == 1 && i == 1
            ...
            %%% interpolate diffusion at the interface
            Dxint = (Dx(j,i+1)-Dx(j,i))*r + Dx(j,i);
            %%% interface very close to index2(m)=1 outer boundary
            Vx(j,i) = Dxint/(out-bdy)*(bdy-C(j,i))/(r*dx);
            Vx(j,i+1) = Vx(j,i);
        elseif Mark(j,i) == 1 && i == Nx+1
            r = Phi(j,i) / (Phi(j,i)-Phi(j,i-1));
            %%% interpolate diffusion at the interface
            Dxint = -(Dx(j,i)-Dx(j,i-1))*r + Dx(j,i);
            %%% interface very close to index2(m)=N+1 outer boundary
            Vx(j,i) = Dxint/(out-bdy)*(C(j,i)-bdy)/(r*dx);
            Vx(j,i-1) = Vx(j,i);
        elseif Mark(j,i) == 1 && Phi(j,i-1) < 0
            ...
            if r <= Tol_r %%% interface very close to index2(m)
                Vx(j,i) = Dxint/(out-bdy)*(C(j,i+1)-bdy)/((1+r)*dx);
                Vx(j,i-1) = Vx(j,i);
            else
                Vx(j,i) = Dxint/(out-bdy)*(C(j,i)-bdy)/(r*dx);
                Vx(j,i-1) = Vx(j,i);
        ...
        elseif Mark(j,i) == 1 && Phi(j,i+1) < 0
            ...
            Dxint = (Dx(j,i+1)-Dx(j,i))*r + Dx(j,i);
            if r <= Tol_r %%% interface very close to index2(m)
                Vx(j,i) = Dxint/(out-bdy)*(bdy-C(j,i-1))/((1+r)*dx);
                Vx(j,i+1) = Vx(j,i);
            else
                Vx(j,i) = Dxint/(out-bdy)*(bdy-C(j,i))/(r*dx);
                Vx(j,i+1) = Vx(j,i);
        ...
        elseif Mark(j,i) == 0.5
            ...
            %%% interpolate diffusion at the interface
            Dxint1 = -(Dx(j,i)-Dx(j,i-1))*r1 + Dx(j,i);
            if r1 == 0
                vel1 = 0;
            else
                vel1 = Dxint1/(out-bdy)*(C(j,i)-bdy)/(r1*dx);
            end
            r2 = - Phi(j,i) / (Phi(j,i+1)-Phi(j,i));
            %%% interpolate diffusion at the interface
            Dxint2 = (Dx(j,i+1)-Dx(j,i))*r2 + Dx(j,i);
            if r2 == 0
                vel2 = 0;
            else
                vel2 = Dxint2/(out-bdy)*(bdy-C(j,i))/(r2*dx);
            end
            Vx(j,i-1) = vel1;
            Vx(j,i+1) = vel2;
            if r1 < r2
    ...
    for m = 1:k
        ...
        elseif Mark(j,i) == 1 && Phi(j-1,i) < 0
            ...
            if r <= Tol_r %%% interface very close to index1(m)
                Vy(j,i) = Dyint/(out-bdy)*(C(j+1,i)-bdy)/((1+r)*dy);
                Vy(j-1,i) = Vy(j,i);
            else
                Vy(j,i) = Dyint/(out-bdy)*(C(j,i)-bdy)/(r*dy);
                Vy(j-1,i) = Vy(j,i);
        ...
        elseif Mark(j,i) == 1 && Phi(j+1,i) < 0
            ...
            Dyint = (Dy(j+1,i)-Dy(j,i))*r+Dy(j,i);
            if r <= Tol_r %%% interface very close to index1(m)
                Vy(j,i) = Dyint/(out-bdy)*(bdy-C(j-1,i))/((1+r)*dy);
                Vy(j+1,i) = Vy(j,i);
            else
                Vy(j,i) = Dyint/(out-bdy)*(bdy-C(j,i))/(r*dy);
                Vy(j+1,i) = Vy(j,i);
        ...
        elseif Mark(j,i) == 0.5
            ...
            %%% interpolate diffusion at the interface
            Dyint1 = -(Dy(j,i)-Dy(j-1,i))*r1+Dy(j,i);
            if r1 == 0
                vel1 = 0;
            else
                vel1 = Dyint1/(out-bdy)*(C(j,i)-bdy)/(r1*dy);
            end
            r2 = -Phi(j,i)/(Phi(j+1,i)-Phi(j,i));
            %%% interpolate diffusion at the interface
            Dyint2 = (Dy(j+1,i)-Dy(j,i))*r2 + Dy(j,i);
            if r2 == 0
                vel2 = 0;
            else
                vel2 = Dyint2/(out-bdy)*(bdy-C(j,i))/(r2*dy);
            end
            Vy(j-1,i) = vel1;
            Vy(j+1,i) = vel2;
            if r1 < r2
...
if k > 0
    for m = 1:k
        ...
        if Mark(j,i) == 1 && i == 1
            ...
            %%% interpolate diffusion at the interface
            Dxyint = (sqrt((Dx(j+1,i+1)-Dx(j,i))^2 + (Dy(j+1,i+1)-Dy(j,i))^2)*r ...
                + sqrt(Dx(j,i)^2+Dy(j,i)^2))/sqrt(2);
            %%% interface very close to index2(m)=1 outer boundary
            Veta(j,i) = Dxyint/(out-bdy)*(bdy-C(j,i))/(r*sqdxdy);
            Veta(j+1,i+1) = Veta(j,i);
        end
        if Mark(j,i) == 1 && i == Nx+1
            r = Phi(j,i) / (Phi(j,i)-Phi(j-1,i-1));
            %%% interpolate diffusion at the interface
            Dxyint = (- sqrt((Dx(j,i)-Dx(j-1,i-1))^2 + (Dy(j,i)-Dy(j-1,i-1))^2)*r ...
                + sqrt(Dx(j,i)^2+Dy(j,i)^2))/sqrt(2);
            %%% interface very close to index2(m)=N+1 outher boundary
            Veta(j,i) = Dxyint/(out-bdy)*(C(j,i)-bdy)/(r*sqdxdy);
            Veta(j-1,i-1) = Veta(j,i);
        end
        if Mark(j,i) == 1 && i~=1 && j~=1 && Phi(j-1,i-1) < 0
            ...
            Dxyint = (- sqrt((Dx(j,i)-Dx(j-1,i-1))^2 + (Dy(j,i)-Dy(j-1,i-1))^2)*r ...
                + sqrt(Dx(j,i)^2+Dy(j,i)^2))/sqrt(2);
            if r <= Tol_r %%% interface very close to index2(m)
                Veta(j,i) = Dxyint/(out-bdy)*(C(j+1,i+1)-bdy)/((1+r)*sqdxdy);
                Veta(j-1,i-1) = Veta(j,i);
            else
                Veta(j,i) = Dxyint/(out-bdy)*(C(j,i)-bdy)/(r*sqdxdy);
                Veta(j-1,i-1) = Veta(j,i);
        ...
        if Mark(j,i)==1 && i~=Nx+1 && j~=Nx+1 && j~=1 && i~=1 ...
            ...
            Dxyint = (sqrt((Dx(j+1,i+1)-Dx(j,i))^2 + (Dy(j+1,i+1)-Dy(j,i))^2)*r ...
                + sqrt(Dx(j,i)^2+Dy(j,i)^2))/sqrt(2);
            if r <= Tol_r  %%% interface very close to index2(m)
                Veta(j,i) = Dxyint/(out-bdy)*(bdy-C(j-1,i-1))/...
                    ((1+r)*sqdxdy);
                Veta(j+1,i+1) = Veta(j,i);
            else
                Veta(j,i) = Dxyint/(out-bdy)*(bdy-C(j,i))/(r*sqdxdy);
        ...
        if Mark(j,i) == 0.5
            ...
            %%% interpolate diffusion at the interface
            Dxyint1 = (- sqrt((Dx(j,i)-Dx(j-1,i-1))^2 + (Dy(j,i)-Dy(j-1,i-1))^2)*r1 ...
                + sqrt(Dx(j,i)^2+Dy(j,i)^2))/sqrt(2);
            if r1 == 0
                vel1 = 0;
            else
                vel1 = Dxyint1/(out-bdy)*(C(j,i)-bdy)/(r1*sqdxdy);
            end
            r2 = - Phi(j,i) / (Phi(j+1,i+1)-Phi(j,i));
            %%% interpolate diffusion at the interface
            Dxyint2 = (sqrt((Dx(j+1,i+1)-Dx(j,i))^2 + (Dy(j+1,i+1)-Dy(j,i))^2)*r2 ...
                + sqrt(Dx(j,i)^2+Dy(j,i)^2))/sqrt(2);
            if r2 == 0
                vel2 = 0;
            else
                vel2 = Dxyint2/(out-bdy)*(bdy-C(j,i))/(r2*sqdxdy);
            end
            Veta(j-1,i-1) = vel1;
            Veta(j+1,i+1) = vel2;
            if r1 < r2
...
%             if abs(Veta(j,i)) > 10^10
%                 disp('ERROR in cartesian_velocity -- 3')
%                 return
%             end
        end
    end
...
if k > 0
    for m = 1:k
        ...
        if Mark(j,i) == 1 && j == 1
            r = - Phi(j,i)/(Phi(j+1,i-1)-Phi(j,i));
            %%% interpolate diffusion at the interface
            Dyxint = (sqrt((Dx(j+1,i-1)-Dx(j,i))^2 + (Dy(j+1,i-1)-Dy(j,i))^2)*r ...
                + sqrt(Dx(j,i)^2+Dy(j,i)^2))/sqrt(2);
            %%% interface very close to index1(m)=1 outer boundary
            Vzeta(j,i) = Dyxint/(out-bdy)*(bdy-C(j,i))/(r*sqdxdy);
            Vzeta(j+1,i-1) = Vzeta(j,i);
        end
        if Mark(j,i) == 1 && j == Ny+1
            r = Phi(j,i) / (Phi(j,i)-Phi(j-1,i+1));
            %%% interpolate diffusion at the interface
            Dyxint = (- sqrt((Dx(j,i)-Dx(j-1,i+1))^2 + (Dy(j,i)-Dy(j-1,i+1))^2)*r ...
                + sqrt(Dx(j,i)^2+Dy(j,i)^2))/sqrt(2);
            %%% interface very close to index1(m)=N+1 outer boundary
            Vzeta(j,i) = Dyxint/(out-bdy)*(C(j,i)-bdy)/(r*sqdxdy);
            Vzeta(j-1,i+1) = Vzeta(j,i);
        end
        if Mark(j,i) == 1 && Phi(j+1,i-1) < 0
            ...
            Dyxint = (- sqrt((Dx(j,i)-Dx(j+1,i-1))^2 + (Dy(j,i)-Dy(j+1,i-1))^2)*r ...
                + sqrt(Dx(j,i)^2+Dy(j,i)^2))/sqrt(2);
            if r <= Tol_r %%% interface very close to index1(m)
                Vzeta(j,i) = Dyxint/(out-bdy)*(C(j-1,i+1)-bdy)/...
                    ((1+r)*sqdxdy);
                Vzeta(j+1,i-1) = Vzeta(j,i);
            else
                Vzeta(j,i) = Dyxint/(out-bdy)*(C(j,i)-bdy)/(r*sqdxdy);
        ...
        if Mark(j,i) == 1 && Phi(j-1,i+1) < 0
            ...
            Dyxint = (sqrt((Dx(j-1,i+1)-Dx(j,i))^2 + (Dy(j-1,i+1)-Dy(j,i))^2)*r ...
                + sqrt(Dx(j,i)^2+Dy(j,i)^2))/sqrt(2);
            if r <= Tol_r  %%% interface very close to index1(m)
                Vzeta(j,i) = Dyxint/(out-bdy)*(bdy-C(j+1,i-1))/...
                    ((1+r)*sqdxdy);
                Vzeta(j-1,i+1) = Vzeta(j,i);
            else
                Vzeta(j,i) = Dyxint/(out-bdy)*(bdy-C(j,i))/(r*sqdxdy);
        ...
        if Mark(j,i) == 0.5
            r1 = Phi(j,i)/(Phi(j,i)-Phi(j+1,i-1));
            %%% interpolate diffusion at the interface
            Dyxint1 = (- sqrt((Dx(j,i)-Dx(j+1,i-1))^2 + (Dy(j,i)-Dy(j+1,i-1))^2)*r1 ...
                + sqrt(Dx(j,i)^2+Dy(j,i)^2))/sqrt(2);
            if r1 == 0
                vel1 = 0;
            else
                vel1 = Dyxint1/(out-bdy)*(C(j,i)-bdy)/(r1*sqdxdy);
            end
            r2 = -Phi(j,i)/(Phi(j-1,i+1)-Phi(j,i));
            %%% interpolate diffusion at the interface
            Dyxint2 = (sqrt((Dx(j-1,i+1)-Dx(j,i))^2 + (Dy(j-1,i+1)-Dy(j,i))^2)*r2 ...
                + sqrt(Dx(j,i)^2+Dy(j,i)^2))/sqrt(2);
            if r2 == 0
                vel2 = 0;
            else
                vel2 = Dyxint2/(out-bdy)*(bdy-C(j,i))/(r2*sqdxdy);
            end
            Vzeta(j+1,i-1) = vel1;
            Vzeta(j-1,i+1) = vel2;
            if r1 < r2
